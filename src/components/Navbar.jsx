              ...
              <div className="hidden md:centered-row gap-6">
                  {
                    ['Features','Prices', 'Testimonials'].map((item, index)=>(
                      <a key={index} href="navLink" className="text-sm text-zinc-300/90 hover:text-indigo-300 hover:translate-y-1 ease-out transition-all duration-300" >{item}</a>
                    ))
                  }
              {/* Button */}
                <button className="clash-display text-base bg-linear-to-r from-indigo-400 to-indigo-600 px-4 py-2 rounded-full cursor-pointer hover:-translate-y-0.5 duration-200 ease-out transition-all hover:shadow-xl hover:shadow-indigo-900">
                  Start Trading
                </button>
              </div>
              {/* Mobile Navigation */}
